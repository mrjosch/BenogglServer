...
public class Lobby3 extends Lobby {
    ...
    public void handleJoinRequest(JoinRequest request, Connection con) {
        ...
        if (game.getUsers() < 3 && !game.isRunning()) {
    ...
    public void broadcastLobbySnapshot() {
        if (game.getUsers() == 3) {
            ...
            for (int i = 0; i < game.getUsers(); i++) {
                ...
                unicast(mUser,new GameStart(mUser.getPlayer().getHand(), COUNTDOWN, reizenPoints,game.getUser(0).getName(),game.getUser(1).getName(),game.getUser(2).getName() ,null,new Next(game.getTurn().getName(),"reizen")));
            }
